...
/************************************************/
    ...
    var opened = false;
    $("#schema_content").hide();
    $("#schema_tab").click(function(){
        if(opened){
            $("#schema_tab").animate({"min-height": "2%"});            
            $("#schema_content").hide("slow");
            $('#makeSwitch').bootstrapSwitch('setState', true);
            
        }else{            
            $("#schema_tab").animate({"min-height": "85%"});                                    
            $("#schema_content").show();                        
            $('#makeSwitch').bootstrapSwitch('setState', false);
    ...
    var potSourceEndpoint = {
       ...
       isSource:true,
       scope:"test",
       connectorStyle:{ strokeStyle:sourceColor, lineWidth:14 },
    ...
    var ledIntTargetEndpoint = {
       ...
       scope:"test",
       connectorStyle:{ strokeStyle:targetColor, lineWidth:14 },
    ...
    jsPlumb.bind("jsPlumbConnectionDetached", function(info) {
        ...
        if (whatTarget === "ledFreqModule" || whatTarget === "ledIntModule" || whatTarget === "stateSelect") {
            ...
            if (whatSource == "potFilterValueModule") {
                    $("#spkOscSelect").val("value", "square");
                    $("#spkFreqSlider").slider("value", "0");
                    currentOSC.stop();                    
                    potSourceParameter.off();            
                    jsPlumb.detachAll(whatTarget);
            } else if (whatSource == "ldrFilterValueModule") {                    
    ...
    $("#makeSwitch").on('switch-change', function (e, data) {
        var $el = $(data.el)
        , value = data.value;
        if (value == false) {
            opened = false;
