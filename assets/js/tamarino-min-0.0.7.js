...
/************************************************/
    ...
    function onReady(event) {
        ...
        // for (var i = 3; i < 14; i++) {
        //   spks.push(new SPK(arduino, arduino.getDigitalPin(i)));
        // }
        ...
        // speaker = new SPK(arduino, arduino.getDigitalPin(9));

        // speaker = arduino.getDigitalPin(6);
        ...
        // servo = new Servo(arduino, arduino.getDigitalPin(11));
        // servo.angle = 10;
    ...
    function ldrActive(value) {        
        if (ldrs[0] == null) {
            ...
            // } else {
            //     arduino.disableAnalogPin(ldrs[0]);                
            //     arduino.disableAnalogPin(ldrs[1]);
            //     ldrs[0] = ldrs[1];
            //     ldrs.length = 1;
            // }                    
        }
    ...
    function setLedPort(value) {
        ...
        if (portsLed[0] == null) {
            portsLed.push(value);
            // arduino.setDigitalPinMode(portsLed[0], Pin.DIN);            
            $("#spkPortSelect option[value=" + value + "]").attr('disabled','disabled');                        
            $("#srvPortSelect option[value=" + value + "]").attr('disabled','disabled');                        
            $("#btnPortSelect option[value=" + value + "]").attr('disabled','disabled');                        
        } else {
            portsLed.push(value);
            // arduino.setDigitalPinMode(portsLed[0], Pin.DOUT);
            // arduino.setDigitalPinMode(portsLed[1], Pin.DIN);                                    
    ...
    $( ".moduleLed").on("click", function (){
        ...
        jsPlumb.addEndpoint($(".stateSelect") , { anchor:[0, 0.35, 0, 1]}, ledStateTargetEndpoint);                
    ...
    $( ".moduleBtn").on("click", function (){
        ...
        jsPlumb.animate($(".btn"), {"left": 750,"top": 450},{duration:"slow"});        
    ...
    $( ".moduleSrv").on("click", function (){
        ...
        jsPlumb.animate($(".srv"), {"left": 350,"top": 450},{duration:"slow"});        
    ...
    $('#trash').droppable({
        over: function(event, ui) {
            ...
            var ledEndPointState = $(ui.draggable).find('div#radio').parent();
            var ledEndPointStateParent = ledEndPointState.parent();
            ...
            if (potEndPointClass == true) {
                ...
                jsPlumb.selectEndpoints({element:ledEndPointStateParent}).setVisible(false);
                $("canvas#output").setLayer("led", {
                    visible: false                
                    }).drawLayers();
                $("canvas#schematico").animateLayer(2, {
...
// BACK-END jSPLUMB DATAFLOW UI
    ...
    $("canvas#input-btn").addLayer({
        method: "drawImage",
        source: "libs/canvas/button_off.png",
        rotate: 180,
        x: 214.5, y: 398,
        // layer: true,
        visible: true,
      ...
      method: "drawImage",
        source: "libs/canvas/resistor_1k.png",
        x: 185, y: 421,
        // layer: true,
        visible: true,
        name: "resistor_22k"
    ...
    // $("#schema_content").css('-moz-transform', 'scale(' + scale + ')');
